...
tdate = getDateRange()
# 用sqlalchemy创建引擎
sql = "select * from city_wcdma_day where 地市 <>'其他'  AND   日期>= %s AND 日期 <  %s" % (
...
excelFileName = filePath + tdate['startDate'] + '_' + tdate['today'] + '_WCDMA.xlsx'  # 文件名称为本月起始日期_结束日期_LTE.xlsx
writer = pd.ExcelWriter(excelFileName)  # 保存表格为excel      文件名称为本月起始日期_结束日期_WCDMA.xlsx
...
class CreateChart:
    def createCharts(self, rrcCity, rowName, figIndex, yRange):
        ...
        self.imageFileName = filePath + tdate['startDate'] + '_' + tdate['endDate'] + '_WCDMA_' + rowName + '.png'  # 保存图表的文件名 为 本月起始日期_结束日期_LTE_KPI名称.PNG图片
        self.rrcFig.savefig(self.imageFileName)  # 保存为 本月起始日期_结束日期_LTE_KPI名称.PNG图片

yRanges = (
(99, 100), (99, 100), (99, 100), (0, 0.5), (0, 0.5), (99, 100), (25, 55), (0, 100), (75, 100), (-110, -100), (0, 10),
(0, 8000), (0, 800000), (0, 100000))
...
if 1 == 1:
    ...
    mailTitle = '3G周报材料_' + tdate['startDate'] + '_' + tdate['today']
    mailBody = 'WCDMA ' + tdate['startDate'] + ' - ' + tdate['today'] + ' 周报材料'
    print(2)
